...
StateWin::StateWin(PokemonGame* pOwner)
	: m_pOwner(pOwner)
	, m_gameObjects()
	, m_keyboardButtonIndex{ 0 }
{
	...
	//-------------------------------------------------------------------------------------------------
	m_gameObjects.push_back(new UITextField(pNavigationFont, kWinNavigationText, kNavigationColor, kWinNavigationPosition, pOwner->GetGame()->GetRenderer()));
...
void StateWin::Update(double deltaTime)
{
